...
const Admin = {
  ...
  permissions: {
    ...
    canViewRevenue: true,
    canManageManagers: false, // فقط المالك
    canExportData: true
  },
  ...
  checkSession: async () => {
    try {
      const {  { session } } = await Admin.db.auth.getSession();
      
      if (session) {
        // التحقق من صلاحية المدير
        const {  profile } = await Admin.db.from('profiles')
  ...
  initMiniMap: () => {
    ...
    // إضافة علامات عشوائية للعرض فقط
  ...
  // 👥 3. إدارة المستخدمين (عام/سائقين/دلفري)
    ...
    try {
      ...
      let html = `
        <div style="display:flex; gap:10px; margin-bottom:20px; flex-wrap:wrap">
          <input type="text" id="user-search" class="input-field" placeholder="🔍 بحث بالاسم أو الهاتف..." 
                 style="flex:1; min-width:200px" onkeyup="Admin.searchUsers()">
        ...
        <div class="table-container">
          <table>
            <thead>
              <tr>
                <th>الاسم</th><th>الهاتف</th><th>الدور</th><th>السن</th><th>الحالة</th><th>تاريخ التسجيل</th><th>إجراءات</th>
              </tr>
            </thead>
      ...
      (data || []).forEach(u => {
        ...
        html += `
          <tr data-name="${u.name?.toLowerCase() || ''}" data-phone="${u.phone || ''}" data-status="${u.status}">
            ...
            <td>
              ...
              <button class="btn-sm ${u.status==='نشط'?'btn-reject':'btn-approve'}" onclick="Admin.toggleUserStatus('${u.id}', '${u.status}')">
                ${u.status==='نشط'?'🚫':'✅'}
              </button>
            </td>
          </tr>
  ...
  openUserModal: async (userId) => {
    try {
      ...
      modal.innerHTML = `
        <div class="modal-content" style="max-width:700px; max-height:90vh; overflow-y:auto">
          ...
          <div style="display:flex; gap:20px; margin-bottom:25px; align-items:center">
            <img src="${user.profile_image || 'https://ui-avatars.com/api/?name='+encodeURIComponent(user.name)}" 
                 style="width:100px; height:100px; border-radius:50%; object-fit:cover; border:3px solid var(--p)">
            <div>
          ...
          ${user.role === 'سائق تكسي' || user.role === 'سائق توك توك' ? `
            <div style="background:#f8fafc; padding:15px; border-radius:10px; margin-bottom:15px">
              ...
              ${user.vehicle_images?.length ? `
                <p style="margin-top:10px"><strong>الصور:</strong></p>
                <div style="display:flex; gap:10px; flex-wrap:wrap">
                  ${user.vehicle_images.slice(0,3).map(img => `<img src="${img}" style="width:80px; height:60px; border-radius:8px; object-fit:cover">`).join('')}
                </div>
              ` : ''}
            </div>
          ` : ''}
          ...
          ${user.role === 'صاحب متجر' ? `
            <div style="background:#f8fafc; padding:15px; border-radius:10px; margin-bottom:15px">
              ...
              ${user.store_images?.length ? `
                <p style="margin-top:10px"><strong>صور المتجر:</strong></p>
                <div style="display:flex; gap:10px; flex-wrap:wrap">
                  ${user.store_images.slice(0,3).map(img => `<img src="${img}" style="width:80px; height:60px; border-radius:8px; object-fit:cover">`).join('')}
                </div>
              ` : ''}
            </div>
          ` : ''}
          ...
          ${user.latitude && user.longitude ? `
            <div style="margin-bottom:15px">
              <h4 style="margin-bottom:10px">📍 الموقع الحالي</h4>
              <div id="modal-map" style="height:200px; border-radius:10px; background:#f1f5f9"></div>
            </div>
          ` : ''}
      ...
      // تحميل الخريطة المصغرة إذا وجد موقع
  ...
  // 📄 تصدير ملف المستخدم لـ PDF
  exportUserPDF: async (userId) => {
    // تحميل مكتبة jsPDF ديناميكياً إذا لم تكن محملة
    ...
    // إعدادات الخط العربي (ملاحظة: يحتاج لخط يدعم العربية)
    doc.setFontSize(18);
    ...
    if (user.vehicle_type) {
      doc.text(`نوع المركبة: ${user.vehicle_type}`, 20, 110);
      doc.text(`رقم اللوحة: ${user.plate_number || '-'}`, 20, 120);
    }
  ...
  editUserProfile: async (userId) => {
    ...
    if (newName === null || newPhone === null) return; // إلغاء
  ...
  manageUserSubscription: async (userId) => {
    const {  user } = await Admin.db.from('profiles').select('subscription_end').eq('id', userId).single();
    ...
    try {
      ...
      let html = `
        <div style="display:flex; gap:10px; margin-bottom:20px">
          <input type="text" id="store-search" class="input-field" placeholder="🔍 بحث باسم المتجر أو الهاتف..." 
                 style="flex:1" onkeyup="Admin.searchStores()">
        </div>
        <div class="table-container">
          <table>
            <thead>
              <tr><th>اسم المتجر</th><th>المالك</th><th>الهاتف</th><th>النوع</th><th>الحالة</th><th>إجراءات</th></tr>
            </thead>
      ...
      (data || []).forEach(s => {
        html += `
          <tr data-name="${(s.name + ' ' + (s.store_type || '')).toLowerCase()}" data-phone="${s.phone || ''}">
            <td><strong>${s.name || '-'}</strong><br><small style="color:var(--g)">${s.store_type || ''}</small></td>
            <td>${s.name || '-'}</td>
            <td>${s.phone || '-'}</td>
            <td>${s.store_type || '-'}</td>
            <td><span class="badge ${s.store_status==='مفتوح'?'badge-success':'badge-warning'}">${s.store_status || 'مغلق'}</span></td>
            <td>
              <button class="btn-sm btn-view" onclick="Admin.openStoreModal('${s.id}')">👁️</button>
              <button class="btn-sm btn-sub" onclick="Admin.editStoreProducts('${s.id}')">📦 منتجات</button>
              <button class="btn-sm ${s.store_status==='مفتوح'?'btn-reject':'btn-approve'}" onclick="Admin.toggleStoreStatus('${s.id}')">
                ${s.store_status==='مفتوح'?'🔴':'🟢'}
              </button>
            </td>
          </tr>
        `;
  ...
  openStoreModal: async (storeId) => {
    const {  store } = await Admin.db.from('profiles').select('*').eq('id', storeId).single();
    if (!store) return;
    ...
    modal.innerHTML = `
      <div class="modal-content" style="max-width:600px">
        <span class="modal-close" onclick="this.closest('.modal').remove()">&times;</span>
        <h3 style="margin-bottom:20px">🏪 ${store.name}</h3>
        <p><strong>📱 الهاتف:</strong> ${store.phone}</p>
        <p><strong>🏷️ النوع:</strong> ${store.store_type}</p>
        <p><strong>📊 الحالة:</strong> ${store.store_status}</p>
        <p><strong>📅 الاشتراك:</strong> ${store.subscription_end ? new Date(store.subscription_end).toLocaleDateString('ar-IQ') : '-'}</p>
        
        ${store.store_images?.length ? `
          <p style="margin-top:15px"><strong>صور المتجر:</strong></p>
          <div style="display:flex; gap:10px; flex-wrap:wrap">
            ${store.store_images.map(img => `<img src="${img}" style="width:100px; height:80px; border-radius:8px; object-fit:cover">`).join('')}
          </div>
        ` : ''}
        
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:20px">
          <button class="btn-sub" onclick="Admin.editStoreProducts('${storeId}')">📦 تعديل المنتجات</button>
          <button class="btn-outline" onclick="Admin.editStoreDetails('${storeId}')">✏️ تعديل البيانات</button>
          <button class="btn-danger" onclick="Admin.deleteStore('${storeId}')">🗑️ حذف</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
  ...
  editStoreProducts: async (storeId) => {
    // تحميل منتجات المتجر
    const {  products } = await Admin.db.from('products').select('*').eq('store_id', storeId);
    
    let html = `
      <div class="modal" id="products-modal">
        <div class="modal-content" style="max-width:700px; max-height:90vh; overflow-y:auto">
          <span class="modal-close" onclick="document.getElementById('products-modal').remove()">&times;</span>
          <h3 style="margin-bottom:20px">📦 منتجات المتجر</h3>
          
          <div style="background:#f8fafc; padding:15px; border-radius:10px; margin-bottom:20px">
            <h4 style="margin-bottom:10px">➕ إضافة منتج جديد</h4>
            <input type="text" id="new-prod-name" class="input-field" placeholder="اسم المنتج" style="margin-bottom:10px">
            ...
            <button class="btn-primary" style="margin-top:10px" onclick="Admin.addProduct('${storeId}')">حفظ المنتج</button>
          </div>
    ...
    (products || []).forEach(p => {
      html += `
        <div class="list-item" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px">
          <div style="display:flex; gap:15px; align-items:center">
            ...
            <div>
              ...
              <span style="color:var(--p); font-weight:bold">${p.price} د.ع</span>
              <span class="badge ${p.status==='متوفر'?'badge-success':'badge-warning'}" style="margin-right:5px">${p.status}</span>
            </div>
          </div>
          <div>
            <button class="btn-sm btn-outline" onclick="Admin.toggleProductStatus('${p.id}')">🔄</button>
            <button class="btn-sm btn-danger" onclick="Admin.deleteProduct('${p.id}')">🗑️</button>
          </div>
        </div>
      `;
    });
  ...
  addProduct: async (storeId) => {
    ...
    // (اختياري) رفع الصورة هنا
    await Admin.db.from('products').insert({
      store_id: storeId,
      name,
      price,
      status: 'متوفر'
    ...
    Admin.editStoreProducts(storeId); // إعادة تحميل القائمة
  },

  toggleProductStatus: async (productId) => {
    const {  prod } = await Admin.db.from('products').select('status').eq('id', productId).single();
  ...
  toggleStoreStatus: async (storeId) => {
    const {  store } = await Admin.db.from('profiles').select('store_status').eq('id', storeId).single();
  ...
  loadTrips: async (container) => {
    try {
      // تصفية الرحلات الأقدم من 30 يوم تلقائياً في العرض
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
      
      const {  data } = await Admin.db.from('trips')
        .select(`
          *,
      ...
      let html = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; flex-wrap:wrap; gap:10px">
          <div>
            <strong>📊 عرض الرحلات (آخر 30 يوم)</strong>
            <p style="color:var(--g); font-size:13px">يتم تصفير هذا القسم تلقائياً كل 30 يوم</p>
          </div>
          <button class="btn-outline" style="width:auto" onclick="Admin.clearOldTrips()">🗑️ مسح الرحلات القديمة يدوياً</button>
        </div>
        <div class="table-container">
          <table>
            <thead>
              <tr><th>الزبون</th><th>النوع</th><th>السائق</th><th>السعر</th><th>الحالة</th><th>التاريخ</th><th>إجراء</th></tr>
            </thead>
            <tbody>
      `;
      ...
      (data || []).forEach(t => {
        ...
        html += `
          <tr>
            <td><strong>${t.customer?.name || 'زبون'}</strong><br><small>${t.customer?.phone || ''}</small></td>
            <td>${t.service_type}</td>
            <td>${t.driver?.name || '-'}</td>
            <td style="font-weight:bold">${t.final_price || 0} د.ع</td>
            <td><span style="color:${statusColors[t.status] || '#64748b'}; font-weight:600">${t.status}</span></td>
            <td>${new Date(t.created_at).toLocaleString('ar-IQ')}</td>
            <td>
              <button class="btn-sm btn-view" onclick="Admin.openTripModal('${t.id}')">👁️</button>
            </td>
          </tr>
        `;
      });
  ...
  openTripModal: async (tripId) => {
    const {  trip } = await Admin.db.from('trips')
      .select(`
        *,
        customer:customer_id(name, phone),
        driver:driver_id(name, phone)
    ...
    modal.innerHTML = `
      <div class="modal-content" style="max-width:500px">
        <span class="modal-close" onclick="this.closest('.modal').remove()">&times;</span>
        <h3 style="margin-bottom:20px">📦 تفاصيل الرحلة #${trip.id?.slice(0,6)}</h3>
        
        <div style="background:#f8fafc; padding:15px; border-radius:10px; margin-bottom:15px">
          <p><strong>👤 الزبون:</strong> ${trip.customer?.name} - ${trip.customer?.phone}</p>
          <p><strong>🚗 النوع:</strong> ${trip.service_type}</p>
          <p><strong>👨‍✈️ السائق:</strong> ${trip.driver?.name || 'لم يتم تعيين سائق'}</p>
          <p><strong>💰 السعر:</strong> ${trip.final_price} د.ع</p>
          <p><strong>📍 من:</strong> ${trip.pickup_lat?.toFixed(4)}, ${trip.pickup_lng?.toFixed(4)}</p>
          <p><strong>🎯 إلى:</strong> ${trip.dropoff_address || trip.dropoff_lat?.toFixed(4)+', '+trip.dropoff_lng?.toFixed(4)}</p>
          <p><strong>⏰ التاريخ:</strong> ${new Date(trip.created_at).toLocaleString('ar-IQ')}</p>
          <p><strong>📊 الحالة:</strong> <span style="color:${trip.status==='مكتملة'?'var(--green)':'var(--p)'}">${trip.status}</span></p>
        </div>
        
        <div style="display:flex; gap:10px; margin-bottom:15px">
          <button class="btn-primary" style="flex:1" onclick="Admin.contactUser('${trip.customer?.phone}')">💬 الزبون</button>
          ${trip.driver?.phone ? `<button class="btn-outline" style="flex:1" onclick="Admin.contactUser('${trip.driver.phone}')">📞 السائق</button>` : ''}
        </div>
        
        <div style="margin-bottom:15px">
          <label><strong>تغيير الحالة:</strong></label>
          <select id="trip-status" class="input-field" style="margin-top:5px">
            <option value="قيد الانتظار" ${trip.status==='قيد الانتظار'?'selected':''}>⏳ قيد الانتظار</option>
            <option value="جاري التنفيذ" ${trip.status==='جاري التنفيذ'?'selected':''}>🚗 جاري التنفيذ</option>
            <option value="مكتملة" ${trip.status==='مكتملة'?'selected':''}>✅ مكتملة</option>
            <option value="ملغاة" ${trip.status==='ملغاة'?'selected':''}>❌ ملغاة</option>
          </select>
        </div>
        
        <button class="btn-primary" onclick="Admin.updateTripStatus('${tripId}')">💾 حفظ التغييرات</button>
      </div>
    `;
    document.body.appendChild(modal);
  },
  ...
  // 💰 6. الإيرادات والاشتراكات
  loadRevenue: async (container) => {
    try {
      // حساب الإيرادات: 30,000 دينار/شهر لكل سائق/دلفري نشط
      ...
      container.innerHTML = `
        <div class="stats-grid">
          <div class="stat-card" style="border-color:#10b981">
            <h3>🚗 سائقين التكسي النشطين</h3>
            <div class="value">${activeTaxis}</div>
            <p style="color:var(--g); font-size:13px; margin-top:5px">الإيراد: ${activeTaxis * subscriptionFee.toLocaleString()} د.ع</p>
          </div>
          <div class="stat-card" style="border-color:#f59e0b">
            <h3>🛺 سائقين التوك توك النشطين</h3>
            <div class="value">${activeTuks}</div>
            <p style="color:var(--g); font-size:13px; margin-top:5px">الإيراد: ${activeTuks * subscriptionFee.toLocaleString()} د.ع</p>
          </div>
          <div class="stat-card" style="border-color:#ef4444">
            <h3>🏍️ الدلفري النشطين</h3>
            <div class="value">${activeDeliveries}</div>
            <p style="color:var(--g); font-size:13px; margin-top:5px">الإيراد: ${activeDeliveries * subscriptionFee.toLocaleString()} د.ع</p>
          </div>
          <div class="stat-card" style="border-color:#8b5cf6; background:linear-gradient(135deg, #8b5cf6, #6366f1)">
            <h3>💰 إجمالي الإيرادات الشهرية</h3>
            <div class="value" style="color:white">${totalRevenue.toLocaleString()} د.ع</div>
            <p style="color:rgba(255,255,255,0.9); font-size:13px; margin-top:5px">${totalActive} مشترك نشط × 30,000 د.ع</p>
          </div>
        </div>
        
        <div class="table-container" style="margin-top:20px">
          <h4 style="margin-bottom:15px">📋 تفاصيل المشتركين</h4>
          <table>
            <thead><tr><th>الاسم</th><th>الهاتف</th><th>الدور</th><th>الحالة</th><th>اشتراك حتى</th><th>الإيراد</th></tr></thead>
            <tbody>
      `;
      ...
      [...(taxis.data||[]), ...(tuks.data||[]), ...(deliveries.data||[])].forEach(u => {
        ...
        container.querySelector('tbody').innerHTML += `
          <tr>
            ...
            <td>${u.phone}</td>
            <td>${u.role}</td>
            <td><span class="badge ${isActive?'badge-success':'badge-warning'}">${u.status}</span></td>
            <td>-</td>
            <td style="font-weight:bold; color:${isActive?'var(--green)':'var(--g)'}">${revenue.toLocaleString()} د.ع</td>
          </tr>
        `;
      });
      
      container.innerHTML += `</tbody></table></div>
        <p style="text-align:center; color:var(--g); margin-top:20px; font-size:13px">
          💡 الإيرادات تحسب تلقائياً بناءً على عدد السائقين والدلفري النشطين × 30,000 د.ع شهرياً
        </p>
      `;
    } catch (err) {
      console.error('Revenue error:', err);
      container.innerHTML = '<div class="text-center mt-2" style="color:var(--red)">❌ فشل تحميل الإيرادات</div>';
    }
  },

  // 🔔 7. الإشعارات والرسائل
  loadNotifications: async (container) => {
    // محاكاة بيانات الإشعارات (يمكن ربطها بجدول حقيقي لاحقاً)
    const notifications = [
    ...
    let html = `
      <div style="margin-bottom:20px">
        <input type="text" id="notif-search" class="input-field" placeholder="🔍 بحث باسم المستخدم أو الهاتف..." onkeyup="Admin.searchNotifications()">
      </div>
      <div class="table-container">
        <table>
          <thead><tr><th>المستخدم</th><th>الهاتف</th><th>الرسالة</th><th>الوقت</th><th>الحالة</th><th>رد</th></tr></thead>
          <tbody id="notif-body">
    `;
    
    notifications.forEach(n => {
      html += `
        <tr data-user="${n.user.toLowerCase()}" data-phone="${n.phone}">
          <td><strong>${n.user}</strong></td>
          <td>${n.phone}</td>
          <td>${n.message}</td>
          <td>${n.time}</td>
          <td><span class="badge ${n.read?'badge-success':'badge-warning'}">${n.read?'مقروء':'جديد'}</span></td>
          <td>
            <button class="btn-sm btn-primary" onclick="Admin.replyToNotification('${n.phone}', '${n.user}')">💬 رد</button>
          </td>
        </tr>
      `;
    });
  ...
  loadManagers: async (container) => {
    ...
    // جلب المدراء الحاليين (محاكاة - يمكن ربطها بجدول admins)
    const managers = [
    ...
    let html = `
      <div style="background:#f8fafc; padding:20px; border-radius:12px; margin-bottom:20px">
        ...
        <input type="email" id="new-manager-email" class="input-field" placeholder="البريد الإلكتروني" style="margin-bottom:10px">
        <input type="text" id="new-manager-name" class="input-field" placeholder="الاسم" style="margin-bottom:10px">
        
        <div style="margin:15px 0">
          <label><input type="checkbox" id="perm-users"> إدارة المستخدمين</label><br>
          <label><input type="checkbox" id="perm-stores"> إدارة المتاجر</label><br>
          <label><input type="checkbox" id="perm-revenue"> عرض الإيرادات</label>
        </div>
        
        <button class="btn-primary" onclick="Admin.addNewManager()">إضافة المدير</button>
      </div>
      
      <div class="table-container">
        <table>
          <thead><tr><th>الاسم</th><th>البريد</th><th>صلاحيات المستخدمين</th><th>صلاحيات المتاجر</th><th>صلاحيات الإيرادات</th><th>إجراءات</th></tr></thead>
          <tbody>
    `;
    ...
    managers.forEach(m => {
      html += `
        <tr>
          <td>${m.name}</td>
          <td>${m.email}</td>
          <td><span class="badge ${m.permissions.users?'badge-success':'badge-warning'}">${m.permissions.users?'✅':'❌'}</span></td>
          <td><span class="badge ${m.permissions.stores?'badge-success':'badge-warning'}">${m.permissions.stores?'✅':'❌'}</span></td>
          <td><span class="badge ${m.permissions.revenue?'badge-success':'badge-warning'}">${m.permissions.revenue?'✅':'❌'}</span></td>
          <td>
            <button class="btn-sm btn-outline" onclick="Admin.editManager('${m.email}')">✏️</button>
            <button class="btn-sm btn-danger" onclick="Admin.removeManager('${m.email}')">🗑️</button>
          </td>
        </tr>
      `;
    });
  ...
  addNewManager: async () => {
    ...
    // هنا يتم إنشاء الحساب في auth.users وإضافة صلاحياته
    alert('✅ تم إرسال دعوة للمدير الجديد (تجريبي)');
    Admin.render('managers');
  },

  editManager: (email) => {
    alert('تعديل صلاحيات: ' + email + '\n(واجهة تجريبية)');
  },

  removeManager: (email) => {
    if (confirm(`إزالة المدير ${email}؟`)) {
      alert('✅ تم إزالة المدير');
      Admin.render('managers');
    }
  },
  ...
  // 🔧 دوال مساعدة
  init: () => {
    // تهيئة المكتبات الخارجية إذا لزم
